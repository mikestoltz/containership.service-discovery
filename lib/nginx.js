            ...
            async.parallel({
                ...
                async.each(response.applications, (application, callback) => {
                    application.containers = _.map(application.containers, (container) => {
                        if (container && container.host && response.hosts[container.host] && container.status === 'loaded'
                            && (!application.health_check || (container.tags && container.tags.metadata && container.tags.metadata.healthy >= application.health_check.healthy_threshold))) {
                            return {
